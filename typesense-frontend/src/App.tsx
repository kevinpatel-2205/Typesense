...
export default function App() {
  ...
  }, [state.q, state.category, state.status, state.tags, state.location,
    ...
    <div className="min-h-screen bg-bg text-text">
      <header className="border-b border-border px-8 py-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted font-mono mt-0.5">faceted hybrid search · 10,00,000 records</p>
        </div>
      ...
      <div className="flex">
        ...
        <main className="flex-1 px-6 py-5 flex flex-col gap-4 min-w-0">
          ...
          {(state.category || state.status || state.tags || state.location || state.dateFrom) && (
            <div className="flex gap-2 flex-wrap">
              {state.category && state.category.split(',').map((c) => (
                <span key={c} className="text-xs font-mono px-2 py-1 bg-accent/10 text-accent rounded border border-accent/20">
                  {c.trim()}
                </span>
              ))}
              {state.status && state.status.split(',').map((s) => (
                <span key={s} className="text-xs font-mono px-2 py-1 bg-surface text-muted rounded border border-border">
                  {s.trim()}
                </span>
              ))}
              {state.location && (
